...
function createLayer(layerConfig) {
     ...
     function createWMTSLayerAsync(layerConfig) {
        ...
        let l = fetch(layerConfig.url + 'request=getcapabilities&SERVICE=WMTS').then(function(response) {
            ...
            let layer = new TileLayer({
                ...
                zIndex: 10,
                //visible: layerConfig.options.visible !== undefined ? layerConfig.options.visible : false,
            });
            for (let k in layerConfig.options) {
                layer.values_[k] = layerConfig.options[k];
            }
    ...
    // Create WMTS Layer werkt niet, projectie gaat verkeerd... //
    function createWMTSLayer(layerConfig) {
        ...
        let layer = new TileLayer({
            ...
            zIndex: layerConfig.zIndex !== undefined ? layerConfig.zIndex : 10,
          });
          return layer;
    }
