...
import {buildLayerManager} from './layerManager';
...
GeoApp.init = function () {
  ...
  for (let key in GeoApp.applicatieSettings.overLays) {
    map.addLayer(createLayer(GeoApp.applicatieSettings.overLays[key]));
  }
  ...
  function addBaseLayers(mapTarget, layerCollection) {
    ...
    for (let key in baseLayerArray) {
      ...
      i === 1 ? baseLayerArray[key].options.visible = true : baseLayerArray[key].options.visible = false;
      mapTarget.addLayer(createLayer(baseLayerArray[key]));
      i++;
    }
    addBaseLayerButton(layerCollection);
  }
};
