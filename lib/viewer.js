...
GeoApp.init = function () {
  ...
  GeoApp.applicatieSettings.authorisation !== undefined? forceAuthorisation(GeoApp.applicatieSettings.authorisation.serverUrl, GeoApp.applicatieSettings.authorisation.layerName) : false ;
  ...
  function forceAuthorisation(url, layer) {
    let request = url + `/wms?request=getMap&LAYERS=${layer}&BBOX=134249.8,448949.6,134275.1,448968.5&WIDTH=10&HEIGHT=10&format=image/png`;
    fetch(request);
  }
};
