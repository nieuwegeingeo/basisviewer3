...
function createLayerLegend(layerItem, layer, map) {
    ...
    let layerLegendUrl = `${layerUrl}?request=getLegendGraphic&format=image/png&LAYER=${layerParam}&scale=${legendScale === true ? getScale(map) : ''}`;
    ...
    if (legendScale === true) {
        map.on('moveend', function() {
            layerLegendUrl = `${layerUrl}?request=getLegendGraphic&format=image/png&LAYER=${layerParam}&scale=${getScale(map)}`;
