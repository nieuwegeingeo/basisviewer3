        ...
        if (filterArray !== undefined) {
            ...
            for (let key in filter) {
                ...
                if (filter[key].TYPE.toLowerCase() === 'text') {
                    filterContent += `<br><label class="filter-item"><span class="filter-item-title">${filter[key].TITLE}:</span><input class="filter-input-text" name="${key}" type="${filter[key].TYPE}"></label>`;
            ...
            let filterNodes = document.querySelectorAll(`.filter-body-${layer.ol_uid} input`);
        ...
        if (i === 0) {
            if (value === '') {
                ...
                if (typeof value === 'string') {
                    ...
                    CQL = `strToLowerCase(${key}) IN (${inVal.toLowerCase()})`;
                }
            }
        } else {
            if (value === '') {
                ...
                if (typeof value === 'string') {
                    ...
                    CQL += ` ${andOr} strToLowerCase(${key}) IN (${inVal.toLowerCase()})`;
                }
            }
        }
        n++;
...
function clearFilter(layer, filterButton) {
    let filterObj = GeoApp.filter[layer.ol_uid];
    let filterItems = document.querySelectorAll(`.filter-body-${layer.ol_uid} input:not(.and_or)`);
...
function setAndOr(n) {
    ...
    if (node.type === 'text') {
        ...
        let prevAndOr = node.parentElement.parentElement.previousElementSibling.previousElementSibling;
        if (prevAndOr.classList.contains('and_or_container')) {
            if (node.checked === false && document.querySelectorAll(`[name=${node.name}]:checked`).length === 0) {
                prevAndOr.classList.add('disabled')
            } else {
                prevAndOr.classList.remove('disabled')
            }
        }
    }
